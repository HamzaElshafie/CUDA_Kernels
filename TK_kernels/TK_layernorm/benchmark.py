...
def _load_tk_extension():
    ...
    try:
        return load(
            ...
            # cuGetErrorString is provided by the CUDA driver library (libcuda),
            # but torch's extension build may not link it by default.
            extra_ldflags=[
                "-lcuda",
            ],
            verbose=False,
...
def run(args):
    ...
    for n in seq_lens:
        ...
        if tk_available:
            ...
            # TK computes mean/var in bf16 (and uses bf16 intermediate normalisation),
            # so y can differ more than resid while still being "correct" for the
            # intended bf16 numerics. Keep resid strict and loosen y only.

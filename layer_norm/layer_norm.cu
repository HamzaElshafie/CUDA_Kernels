...
Steps:
1. Find the sum of all elements in block using sum_reduction
2. Divide the block sum by N to obtain block average
3. Calculate (xi - block_average)^2 and perform another sum reduction across all threads
4. Calculate the inverse standard deviation + epsilon
...
__global__ void layer_norm_f32(float* x, float* y, float eps, int M, int K, float gamma, float beta) {
    ...
    float global_sum = block_reduce_sum_f32<NUM_THREADS>(val);

    if (threadIdx.x == 0) {
        s_mean = global_sum / float(K);
    }
    __syncthreads();

    float variance = (val - s_mean) * (val - s_mean);
